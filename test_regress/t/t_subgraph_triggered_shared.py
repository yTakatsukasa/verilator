...
test.file_grep(test.stats, r"Scheduling, Subgraph schedule equivalence potential reuses\s+(\d+)",
               11)
test.file_grep(test.stats,
               r"Scheduling, Subgraph schedule equivalence representative order calls\s+(\d+)", 8)
...
test.file_grep(test.stats,
               r"Scheduling, Subgraph shared exact parent domain reused body wrappers\s+(\d+)", 11)
test.file_grep(test.stats,
...
test.file_grep(test.stats,
               r"Scheduling, Subgraph shared logic instance binding checks\s+(\d+)", 11)
test.file_grep(test.stats,
               r"Scheduling, Subgraph shared logic instance binding matches\s+(\d+)", 11)
test.file_grep(test.stats, r"Scheduling, Subgraph shared logic signature builds\s+(\d+)", 8)
test.file_grep(test.stats,
               r"Scheduling, Subgraph shared logic signature builds avoided\s+(\d+)", 11)
test.file_grep(test.stats, r"Scheduling, Subgraph shared logic template analyses\s+(\d+)", 19)
