...
test.file_grep(test.stats, r"Scheduling, Subgraph nba, diagnostic fresh order groups\s+(\d+)", 6)
test.file_grep(test.stats, r"Scheduling, Subgraph nba, internal order aggregate calls\s+(\d+)", 6)
test.file_grep(test.stats, r"Scheduling, Subgraph nba, order cache lookups\s+(\d+)", 0)
