...
test.file_grep(test.stats, r"Scheduling, Subgraph NBA materialized internal uses\s+(\d+)", 6)
test.file_grep(test.stats, r"Scheduling, Subgraph NBA pruned internal uses\s+(\d+)", 2)
...
test.file_grep(test.stats, r"Scheduling, Subgraph order graph contract uses\s+(\d+)", 16)
