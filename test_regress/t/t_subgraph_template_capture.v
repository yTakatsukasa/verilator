...
module t;
  ...
  sg_capture_partial ip (clk, data[6:0], yp);
  ...
  always @(negedge clk) begin
    #1;
    `checkh(y0, 7'(((r0 + 7'h3) ^ 7'h5) + r0))
    `checkh(y1, 7'(((r1 + 7'h3) ^ 7'h5) + r1))
    `checkh(y2, 15'(((r2 + 15'h3) ^ 15'h5) + r2))
    `checkh(ya, ra)
    `checkh(yc, rc)
    `checkh(yp, rp)
...
module sg_capture #(parameter W = 7) (
  ...
  // Reverse source order requires the independent scheduler to sort the dataflow.
  assign y = second + q;
  assign second = first ^ W'(5);
  assign first = q + W'(3);
endmodule
...
module sg_capture_partial (
  ...
  /*verilator subgraph_boundary*/
  logic [6:0] q = 1;
  always @(posedge clk) q[6:1] <= data[6:1];
  assign y = q;
endmodule
