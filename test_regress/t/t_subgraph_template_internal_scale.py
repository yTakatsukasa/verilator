...
for label, define, context_variables in (("small", [], 20),
                                         ("large", ["+define+INTERNAL_LARGE"], 80)):
    ...
    test.file_grep(stats,
                   r"Scheduling, Subgraph V3Ast shared body context variables\s+(\d+)",
                   context_variables)

    parent = tuple(read_stat(stats, name) for name in (
