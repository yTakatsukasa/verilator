...
struct SubgraphInputRefreshTarget final {
    ...
    const AstSenTree* m_senTreep = nullptr;
    const AstSenTree* m_triggerDomainp = nullptr;
};
...
bool inputRefreshTargetMatches(const SubgraphInputRefreshTarget& target, const AstVarScope* inputp,
                               const AstSenTree* senTreep) {
    if (target.m_senTreep != senTreep) return false;
    return std::find(target.m_inputps.begin(), target.m_inputps.end(), inputp)
           != target.m_inputps.end();
}
...
bool canRefreshSubgraphInput(const AstSubgraphInstance* instancep, const AstVarScope* inputp,
                             const AstSenTree* senTreep) {
    const SubgraphRegistry& registry = subgraphRegistry();
    const auto it = registry.m_inputRefreshTargets.find(instancep);
    if (it == registry.m_inputRefreshTargets.end()) return false;
    for (const SubgraphInputRefreshTarget& target : it->second) {
        if (inputRefreshTargetMatches(target, inputp, senTreep)) return true;
    }
    return false;
}
...
void appendSubgraphInputRefreshCalls(AstCFunc* funcp) {
    ...
    uint64_t inputs = 0;
    uint64_t triggeredTargets = 0;
    uint64_t unmatchedInstances = 0;
    for (const auto& pair : registry.m_inputRefreshes) {
        ...
        for (const SubgraphInputRefreshTarget& target : targetIt->second) {
            if (target.m_triggerDomainp) ++triggeredTargets;
    ...
    V3Stats::addStat("Scheduling, Subgraph input refresh triggered targets", triggeredTargets);
    V3Stats::addStat("Scheduling, Subgraph input refresh unmatched instances", unmatchedInstances);
}
...
void rememberSubgraphInputRefreshRequest(const AstSubgraphInstance* instancep,
    ...
    subgraphRegistry().m_inputRefreshes[instancep].insert(inputp);
}
...
SubgraphSchedulePlan buildSubgraphSchedulePlan(
    ...
    if (!funcp) {
        ...
        if (funcp) {
            ...
            if (cacheableArtifact && !matchedOrderCacheEntryp) {
                const SubgraphTriggeredRefInfo parameterizedTriggeredInfo
                ...
                const bool triggeredShareable
                    = SubgraphLoweringState::canShareTriggeredArtifact(scheduledTriggeredInfo);
                const bool triggeredCloneable
                    = SubgraphLoweringState::canCloneTriggeredOrderCacheEntry(
                        parameterizedTriggeredInfo);
                ...
                bucket.m_entries.push_back(SubgraphOrderCacheEntry{
                    ...
                    triggeredCloneable, triggeredShareable,
                    scheduledTriggeredInfo.m_writesDelayedShadow,
                    scheduledTriggeredInfo.m_writesInstanceLocal,
                    scheduledTriggeredInfo.m_writesLocalTemp,
                    scheduledTriggeredInfo.m_writesNonLocal,
                    scheduledTriggeredInfo.m_writesTriggerTemp,
                    scheduledTriggeredInfo.m_writesVlemTemp});
    ...
    const bool cloneableArtifact
        = SubgraphLoweringState::canCloneTriggeredOrderCacheEntry(parameterizedTriggeredInfo);
    const bool triggeredShareableArtifact
        = SubgraphLoweringState::canShareTriggeredArtifact(scheduledTriggeredInfo);
    if (scheduledTriggeredInfo.m_hasTriggered) {
        ...
        if (scheduledTriggeredInfo.m_writesNonLocal) {
            ++state.m_stats.m_triggeredArtifactWritesNonLocal;
        }
        ...
        if (!scheduledTriggeredInfo.m_writesNonLocal) {
            ...
            if (scheduledTriggeredInfo.m_writesInstanceLocal) {
    ...
    plan.m_artifactp = state.makeSubgraphScheduleArtifact(
        ...
        bundleContext.m_currentScopeTailContract, cloneableArtifact,
        scheduledTriggeredInfo.m_hasTriggered, triggeredShareableArtifact, cacheableArtifact);
...
AstSubgraphInstance* materializeSubgraphSchedulePlan(
    ...
    if (plan.m_stlTailWrapperp) {
        ...
        target.m_senTreep = group.m_senTreep;
        target.m_triggerDomainp = instance.m_triggerDomainp;
